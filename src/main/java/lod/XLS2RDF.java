...
//			.addProperty(RDFS.label, model.createLiteral("unit", "en"));
	...
	private static void createRDF(Model model, ArrayList<Map<String,String>> mapList, String datasetName, String city) {
		try {
			...
			Resource dataStructureDefinition = model.createResource(up + "dsd_" + datasetName.replaceAll("平成29年度", ""));
			...
			//DataSetの作成
			String datasetUri = up + "The_list_of_businesses_whose_budgets_are_freely_managed_by_city_and_ward_managers_in_Osaka_(2017)";
			...
			.addProperty(RDFS.label, model.createLiteral("The list of businesses whose budgets are freely managed by city & ward managers in Osaka (2017)","en"))
			...
			for(Map<String,String> map : mapList) {
				...
				for(String manager : managerList) {
					...
					.addProperty(model.getProperty(up + "annualExpenditure"), model.createTypedLiteral(Integer.parseInt(map.get(manager).split("<>")[0]) * 1000))
					.addProperty(model.getProperty(up + "generalRevenue"), model.createTypedLiteral(Integer.parseInt(map.get(manager).split("<>")[1]) * 1000));
				}
			}
...
//			writer.setProperty("allowBadURIs", "true");
			FileOutputStream fout = new FileOutputStream("/Users/Shusaku/Dropbox/research/doctor/osaka_city_manager_budget_20171110.ttl");
...
//			writer.write(model, fout, "TTL");
			...
			for(int i=0; i<sheet.getPhysicalNumberOfRows(); i++) {
				...
				case NUMERIC:
					...
					for(int k=0; k<managerList.size(); k++) {
						rowMap.put(managerList.get(k), 
								row.getCell(k+4).getStringCellValue()
								.replaceAll("\n", "<>")
								.replaceAll("\\(", "")
								.replaceAll("\\)", "")
								.replaceAll(",", ""));
	...
	public static void main(String[] args) {
		String city = "大阪市";
		String datasetName = "平成29年度区シティ・マネージャー自由経費予算事業一覧";
